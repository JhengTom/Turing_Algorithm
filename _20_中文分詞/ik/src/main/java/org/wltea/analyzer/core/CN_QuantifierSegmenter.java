...
public class CN_QuantifierSegmenter implements ISegmenter {
    ...
    // 中文數詞
    private static String Chn_Num = "一二兩三四五六七八九十零壹貳三肆伍陸柒捌玖拾百千萬億拾佰仟萬億兆卅廿"; // Cnum
    ...
    private int nStart;
    /*
     * 記錄詞元結束位置 end記錄的是在詞元中最後一個出現的合理的數詞結束
     */
    private int nEnd;
    ...
    private void processCNumber(AnalyzeContext context) {
        if (nStart == -1 && nEnd == -1) {// 初始狀態
            if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType() && ChnNumberChars.contains(context.getCurrentChar())) {
                // 記錄數詞的起始、結束位置
    ...
    private void processCount(AnalyzeContext context) {
        ...
        if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()) {
                ...
                for (Hit hit : tmpArray) {
                    ...
                    if (hit.isMatch()) {
                        ...
                        Lexeme newLexeme = new Lexeme(context.getBufferOffset(), hit.getBegin(), context.getCursor()
                                - hit.getBegin() + 1, Lexeme.TYPE_COUNT);
                        context.addLexeme(newLexeme);

                        if (!hit.isPrefix()) {// 不是詞首碼，hit不需要繼續匹配，移除
            ...
            } else if (singleCharHit.isPrefix()) {// 首字為量詞首碼
                // 首碼匹配則放入hit列表
                this.countHits.add(singleCharHit);
            }
