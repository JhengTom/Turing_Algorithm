...
public class LetterSegmenter implements ISegmenter {
    ...
    /*
     * 詞元的開始位置， 同時作為子分詞器狀態標識 當start > -1 時，標識當前的分詞器正在處理字元
     */
    private int start;
    /*
     * 記錄詞元結束位置 end記錄的是在詞元中最後一個出現的Letter但非Sign_Connector的字元的位置
     */
    private int end;
    ...
    /**
     * 處理數位字母混合輸出 如：windos2000 | linliangyi2005@gmail.com
     *
     * @param input
     * @param context
     * @return
    ...
    private boolean processMixLetter(AnalyzeContext context) {
        ...
        if (this.start == -1) {// 當前的分詞器尚未開始處理字元
            ...
            if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()
                ...
                // 遇到非Letter字元，輸出詞元
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.start, this.end - this.start + 1,
                        Lexeme.TYPE_LETTER);
    ...
    private boolean processEnglishLetter(AnalyzeContext context) {
        ...
        if (this.englishStart == -1) {// 當前的分詞器尚未開始處理英文字元
            ...
            if (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()) {
                ...
                // 遇到非English字元,輸出詞元
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.englishStart, this.englishEnd - this.englishStart
                        + 1, Lexeme.TYPE_ENGLISH);
                context.addLexeme(newLexeme);
                this.englishStart = -1;
                this.englishEnd = -1;
            }
    ...
    private boolean processArabicLetter(AnalyzeContext context) {
        ...
        if (this.arabicStart == -1) {// 當前的分詞器尚未開始處理數位字元
            if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()) {
                // 記錄起始指標的位置,標明分詞器進入處理狀態
            ...
            if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()) {
                ...
                // //遇到非Arabic字元,輸出詞元
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.arabicStart, this.arabicEnd - this.arabicStart + 1,
                        Lexeme.TYPE_ARABIC);
                context.addLexeme(newLexeme);
                this.arabicStart = -1;
                this.arabicEnd = -1;
