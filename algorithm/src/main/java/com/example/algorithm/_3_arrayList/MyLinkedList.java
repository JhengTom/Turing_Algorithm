...
public class MyLinkedList {
	...
	private ListNode head; //最原始的 LinkedList
	private int size = 0;		//
	...
	public void insertHead(int data){		//插入链表的头部		data就是插入的数据
		...
		head = newNode;    			//將新的LinkList 附值回去head
	...
	public void insertNth(int data,int position){		//插入链表的中间 假设定义在第N个插入 O(n)
		if(position == 0) {		//这个表示插入在头部了
			...
			ListNode cur = head;   //將cur 指向到head
			for(int i = 1;  i < position ; i++){
				cur = cur.next;		//一直往后遍历   p=p->next;  ->是c++里面的往后找指针 懂了!!!就是在往後面找值!!!因為路徑記錄在next內
			}
			ListNode newNode = new ListNode(data);
			//
